...
var template = '/*! reliable.%ext% build:' + package.version + ', %type%. Copyright(c) 2013 Michelle Bu <michelle@michellebu.com> */'
  , prefix = '\n(function(exports){\n'
...
if (!module.parent){
  ...
  builder(args.length ? args : false, { minify:false }, function (err, content) {
    ...
    fs.write(
        fs.openSync(__dirname + '/../dist/reliable.js', 'w')
      , content
      , 0
      , 'utf8'
    );
    console.log('Successfully generated the development build: reliable.js');
  });
  ...
  builder(args.length ? args : false, function (err, content) {
    ...
    fs.write(
        fs.openSync(__dirname + '/../dist/reliable.min.js', 'w')
      , content
      , 0
      , 'utf8'
    );
    console.log('Successfully generated the production build: reliable.min.js');
  });
}
